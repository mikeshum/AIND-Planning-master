...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        1. Checks all actions, then checks if potential parent nodes are in previous S-level
        2. If it is, add to this A-level
        3. Connect new action node to S-nodes in previous S-level as their children, add parents to a-node
        ...
        for action in self.all_actions:
            ...
            # Check if potential parent nodes are in previous S-level
            if a_node.prenodes.issubset(self.s_levels[level]):
                #2. If it is, add to A-level
                action_nodes.append(a_node)

                #3. Connect new action node to S-nodes in previous s-level as children, add parents to a-node
    ...
    def add_literal_level(self, level):
        ...
        1. Go through every a-node in the previous level, checking the s-nodes in effnodes
        2. Add all those s-nodes to a set; add that a-node as a parent to the s-node, and
        make the s-node a child of the a-node
        ...
        state_nodes = set()
        #1. Go through every a-node in previous level
        for a_node in self.a_levels[level-1]:
            # Check the s-nodes in effnodes
            for s_node in a_node.effnodes:
                #2. Add all s-nodes to a set
                state_nodes.add(s_node)

                #Add that a-node as a parent to the s-node, make s-node a child of a-node
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        #Test if action from node a1 negates effect of node a2
        for action_effect in node_a1.action.effect_add:
            if action_effect in node_a2.action.effect_rem:
                return True

        # Test if action from node a2 negates effect of node a1
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        #Test if action from node a1 negates precondition (both pos and neg) for node a2
        for action_effect in node_a1.action.effect_add:
        ...
        # Test if action from node a2 negates precondition (both pos and neg) for node a1
        for action_effect in node_a2.action.effect_add:
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        #Test to see if precondition of node a1 is mutex with a precondition of node a2
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        :return: bool
        """
        #PgNode_S.__eq__ definition: self.is_pos == other.is_pos and self.symbol == other.symbol), test for negation
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        """
        #Check to see if there exists a pair of preconditions in node s1 and s2 that are not mutex
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            ...
            #Start at level 0, if goal state is found then break and move on to next one
            for level in range(len(self.s_levels)):
                if goal_state in self.s_levels[level]:
                    level_sum += level
                    break
